...
nvda_clean <- nvda_data[,-c(1, 18, 25)]
nvda_clean <- na.omit(nvda_clean)

# Split data into training and test sets
set.seed(111111)  # For reproducibility
total_obs <- dim(nvda_clean)[1]
...
train_data <- nvda_clean[1:split_index,]
test_data <- nvda_clean[(split_index + 1):total_obs,]
